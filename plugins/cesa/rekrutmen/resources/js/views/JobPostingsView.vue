    ...
    <Sheet :open="isSheetOpen" @update:open="handleSheetUpdate">
      <SheetContent class="sm:max-w-xl">
        ...
        <div v-else class="flex flex-col h-full overflow-hidden">
          ...
          <form @submit.prevent="submitEdit" class="flex-1 overflow-y-auto p-6 space-y-4">
  ...
  try {
    ...
    if (res.success) {
      if (activeJob.value && activeJob.value.id === job.id) {
        activeJob.value.is_published = !activeJob.value.is_published;
      }
      toastType.value = 'success';
...
const saveEditJob = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      const updatedOrCreated = postings.value.find(j => j.title === title) || postings.value[0];
      if (updatedOrCreated) {
